...
- Home order is now: hero, training, press, Netram row, impact, trust, What I treat, every eye condition, Recognition, social work (Netram / Eye Maitri / Foundation blocks), consultation. The Recognition band keeps its original centred layout (heading, EKG divider, slider); the India map behind it is replaced by the TEDx stage photograph as a faint rose-tinted background (`.band__photo`, the one deliberate cover-fit image on the site, since it is a backdrop); TEDx, CII, BW and Gold Medal slides carry photo avatars. A first attempt that redesigned the band into a photo-mosaic row was rejected by the client the same morning: they had asked for an image, not a new layout.
- "What I treat" tiles and the Expertise cards use six different scenes each (theatre, scrubs, slit lamp, navy scrubs, desk, school camp). The two WhatsApp desk portraits are not in those grids at the client's request: the gold-saree-with-carnations frame sits in "Every eye condition, under one roof" and the hands-folded frame is the front photo of the Netram row.
